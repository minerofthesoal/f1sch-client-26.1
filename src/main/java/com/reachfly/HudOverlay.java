...
public class HudOverlay {
    ...
    private static List<String> getActiveModules() {
        ...
        if (ModConfig.lowHealthKillEnabled) m.add("LowHPKill");
        if (ModConfig.autoKillWhenLowEnabled) m.add("AutoKillLow");
        if (ModConfig.eatingAssistEnabled) m.add("EatingAssist");
        if (ModConfig.autoElytraSwapEnabled) m.add("AutoElytra");
        if (ModConfig.flyToCoordsEnabled) m.add("FlyToCoords");
        if (ModConfig.walkToCoordsEnabled) m.add("WalkToCoords");
        if (ModConfig.autoLogEnabled) m.add("AutoLog");
        if (ModConfig.autoRespawnEnabled) m.add("AutoRespawn");
        if (ModConfig.betterSprintEnabled) m.add("BetterSprint");
        if (ModConfig.safeWalkEnabled) m.add("SafeWalk");
        if (ModConfig.stepEnabled) m.add("Step");
        ...
        if (ModConfig.anchorAuraEnabled) m.add("AnchorAura");
        if (ModConfig.holeFillerEnabled) m.add("HoleFiller");
        if (ModConfig.autoTrapEnabled) m.add("AutoTrap");
        if (ModConfig.reversalEnabled) m.add("Reversal");
        ...
        if (ModConfig.glideEnabled) m.add("Glide");
        if (ModConfig.highJumpEnabled) m.add("HighJump");
        if (ModConfig.dolphinEnabled) m.add("Dolphin");
        if (ModConfig.autoSwordEnabled) m.add("AutoSword");
        if (ModConfig.sneakEnabled) m.add("Sneak");
        if (ModConfig.antiHungerEnabled) m.add("AntiHunger");
        if (ModConfig.triggerBotEnabled) m.add("TriggerBot");
        if (ModConfig.fastPlaceEnabled) m.add("FastPlace");
        if (ModConfig.parkourEnabled) m.add("Parkour");
        if (ModConfig.noSlowdownEnabled) m.add("NoSlowdown");
        if (ModConfig.antiBlindEnabled) m.add("AntiBlind");
        if (ModConfig.autoWalkEnabled) m.add("AutoWalk");
        if (ModConfig.airJumpEnabled) m.add("AirJump");
        if (ModConfig.noWebEnabled) m.add("NoWeb");
        if (ModConfig.flightPlusEnabled) m.add("Flight+");
        if (ModConfig.longJumpEnabled) m.add("LongJump");
        if (ModConfig.autoMLGEnabled) m.add("AutoMLG");
        if (ModConfig.blinkEnabled) m.add("Blink");
        if (ModConfig.baritoneEnabled) m.add("Baritone");
        if (ModConfig.proUnlocked) {
            if (ModConfig.antiKnockbackEnabled) m.add("AntiKB");
            if (ModConfig.noSwingEnabled) m.add("NoSwing");
            if (ModConfig.antiAfkEnabled) m.add("AntiAFK");
            if (ModConfig.fastBreakEnabled) m.add("FastBreak");
            if (ModConfig.nukerEnabled) m.add("Nuker");
            if (ModConfig.autoFarmEnabled) m.add("AutoFarm");
            if (ModConfig.phaseEnabled) m.add("Phase");
            if (ModConfig.freecamEnabled) m.add("Freecam");
            if (ModConfig.timerEnabled) m.add("Timer");
            if (ModConfig.chestEspEnabled) m.add("ChestESP");
            if (ModConfig.trajectoriesEnabled) m.add("Trajectories");
            if (ModConfig.nametagsEnabled) m.add("Nametags");
            if (ModConfig.autoFishEnabled) m.add("AutoFish");
            if (ModConfig.chestStealerEnabled) m.add("ChestStealer");
            if (ModConfig.autoToolEnabled) m.add("AutoTool");
            if (ModConfig.invSortEnabled) m.add("InvSort");
            if (ModConfig.autoBridgeEnabled) m.add("AutoBridge");
            if (ModConfig.towerEnabled) m.add("Tower");
        }
    ...
    private static String getCardinalDirection(float yaw) {
        ...
        if (n >= 315 || n < 45) return "Facing: South (+Z)";
        if (n >= 45 && n < 135) return "Facing: West (-X)";
        if (n >= 135 && n < 225) return "Facing: North (-Z)";
